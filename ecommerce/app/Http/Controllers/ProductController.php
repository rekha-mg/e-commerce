	...
	public function addNewProduct(request $request){
		...
		public function updateProduct(request $request,$product_id){
	        ...
	        if($error){
				...
				$resp1=DB::update('update product set product_name =? ,price=? ,quantity=? ,weight=? where product_id=?',[$product_name,$price,$quantity,$weight,$product_id]);
				Log::info('updated  product table :'.$resp1);
				
			
				$resp2=DB::update('update product_attributes_assoc set  brand =? , manufacture_date=? ,expire_date=? where product_id_fk=?',[$brand,$manufacture_date,$expire_date,$product_id]);
				Log::info('updated new product attributes  assoc:'.$resp2);
				return $this->sendResponse("true", $product_id, 'updated data successfully', 200);

				/*$resp3=DB::insert('insert into product_image(product_id, product_image) values (?,?)',[$product_id,$product_image]);
				Log::info('Inserted new product image '.$resp3);
				return $this->sendResponse("true",$product_id,'data inserted successfully',200);*/

				$resp4=DB::update('update  product_category set category_id=? where product_id_fk=?',[$category_id,$product_id]);
				Log::info('updated new product category :'.$resp4);
